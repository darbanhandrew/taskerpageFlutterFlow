import '/backend/api_requests/api_calls.dart';
import '/flutter_flow/flutter_flow_theme.dart';
import '/flutter_flow/flutter_flow_util.dart';
import '/flutter_flow/custom_functions.dart' as functions;
import 'package:flutter/material.dart';
import 'package:flutter/scheduler.dart';
import 'package:flutter_spinkit/flutter_spinkit.dart';
import 'package:google_fonts/google_fonts.dart';
import 'package:provider/provider.dart';

class SkillLevelSheetModel extends FlutterFlowModel {
  ///  Local state fields for this component.

  String chosenSkillLevel = 'SELF_TRAINED';

  ///  State fields for stateful widgets in this component.

  // Stores action output result for [Backend Call - API (getUserServiceById)] action in SkillLevelSheet widget.
  ApiCallResponse? editableUserService;
  // Stores action output result for [Backend Call - API (editUserService)] action in Container widget.
  ApiCallResponse? editedUserService;

  /// Initialization and disposal methods.

  void initState(BuildContext context) {}

  void dispose() {}

  /// Action blocks are added here.

  /// Additional helper methods are added here.
}
